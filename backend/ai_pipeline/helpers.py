...
def behavioural_context_and_name(body):
    ...
    elif body.type.lower() == "crop":
        q_id = next((k for k, v in q_map.items() if v == "What crop are you growing?"), None)
        crop_name = next((a["answer"] for a in body.answers if a["question_id"] == q_id), None)

    return behavioural_context, animal_name or crop_name
